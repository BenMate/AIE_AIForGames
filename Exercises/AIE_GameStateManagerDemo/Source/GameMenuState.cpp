...
void GameMenuState::Update(float deltaTime)
{
	...
	//takes you to HighScoreState
	if (IsKeyPressed(KEY_TWO))
	{
		m_app->GetGameStateManager()->SetState("Menu", nullptr);
		m_app->GetGameStateManager()->PopState();
		m_app->GetGameStateManager()->PushState("HighScores");
	}
...
void GameMenuState::Draw()
{
	...
	DrawText("Press 1 to guide", 10, 50, 20, GRAY);
	DrawText("Press 2 for highscores ", 10, 100, 20, GRAY);
	DrawText("Press P to Play Game", 10, 150, 20, DARKGRAY);

	DrawText("Press M for Tests", 10, 200, 20, GRAY);
}