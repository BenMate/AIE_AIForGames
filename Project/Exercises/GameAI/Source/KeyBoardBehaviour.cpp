...
KeyBoardBehaviour::KeyBoardBehaviour() : Behaviour() //constructor of derived classes
{
	m_upKey = KEY_W; m_downKey = KEY_S;
	m_leftKey = KEY_A; m_rightKey = KEY_D;
	m_BigboomKey = KEY_SPACE;

	m_moveForce = 500;
}

KeyBoardBehaviour::KeyBoardBehaviour(int upKey, int downKey, int leftKey, int rightKey, float moveForce) :
Behaviour(), m_upKey(upKey), m_downKey(downKey), m_leftKey(leftKey), m_rightKey(rightKey),m_moveForce(moveForce)
{
